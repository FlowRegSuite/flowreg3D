"""
Test fixtures and utilities for FlowReg3D tests.

Provides both 2D and 3D test data generation, fixtures, and utilities
for testing the motion correction pipeline.
"""
...
def create_simple_test_data(
    ...
    for t in range(T):
        for c in range(C):
            ...
            shift_x = t * 2  # Pixels shift per frame
            shift_y = t * 1  # Pixels shift per frame

            # Create sinusoidal pattern
            pattern = np.sin((x + shift_x) * 2 * np.pi / W) * np.sin(
                (y + shift_y) * 2 * np.pi / H
            )
            frames[t, :, :, c] = pattern * 0.5 + 0.5
...
def get_minimal_of_options():
    ...
    return OFOptions(
        ...
        a_smooth=1.0,
        a_data=0.5,
        sigma=[[2.0, 2.0, 0.3], [1.8, 1.8, 0.25]],  # 2 channel sigma values
    ...
    if pattern == "motion":
        ...
        for t in range(T):
            for c in range(C):
                ...
                sphere1 = (
                    (x - center1_x - drift_x) ** 2
                    + (y - center1_y - drift_y) ** 2
                    + (z - center1_z - drift_z) ** 2
                ) <= (min(Z, Y, X) // 8) ** 2

                # Object 2 - sphere
                center2_z, center2_y, center2_x = 3 * Z // 4, 3 * Y // 4, 3 * X // 4
                sphere2 = (
                    (x - center2_x - drift_x) ** 2
                    + (y - center2_y - drift_y) ** 2
                    + (z - center2_z - drift_z) ** 2
                ) <= (min(Z, Y, X) // 10) ** 2

                frames[t, :, :, :, c] = (sphere1 * 0.8 + sphere2 * 0.6).astype(
                    np.float32
                )
...
def create_simple_3d_test_data(
    shape: Tuple[int, int, int, int, int] = (20, 8, 32, 32, 2),
) -> np.ndarray:
    """
...
def create_3d_reference_volume(
    shape: Tuple[int, int, int, int] = (8, 32, 32, 2),
) -> np.ndarray:
    """
...
def get_minimal_3d_of_options():
    ...
    return OFOptions(
        ...
        sigma=[
            [2.0, 2.0, 1.5, 0.3],
            [1.8, 1.8, 1.2, 0.25],
        ],  # 3D sigma [sx, sy, sz, st] per channel
        weight=[1.0, 1.0],  # Equal weight for 2 channels
        min_level=0,
        interpolation_method="cubic",
        update_reference=False,
...
def create_3d_motion_field(
    shape: Tuple[int, int, int, int] = (5, 8, 32, 32), motion_type: str = "translation"
) -> np.ndarray:
    """
...
def apply_3d_motion_to_volume(
    volume: np.ndarray, motion_field: np.ndarray
) -> np.ndarray:
    """
    ...
    for c in range(C):
        ...
        moved_flat = map_coordinates(
            volume[:, :, :, c], coordinates, order=1, mode="reflect"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Created minimal 3D options: alpha={options.alpha}, sigma shape={np.array(options.sigma).shape}"
    )
