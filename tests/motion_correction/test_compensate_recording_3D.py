...
class TestReferenceSetup3D:
    ...
    def test_3d_reference_preprocessing(self, fast_3d_of_options):
        ...
        with (
            patch("flowreg3d.util.image_processing_3D.normalize") as mock_normalize,
            patch(
                "flowreg3d.util.image_processing_3D.apply_gaussian_filter"
            ) as mock_filter,
        ):
...
class TestIO3DIntegration:
    ...
    def test_3d_io_setup(self, fast_3d_of_options, temp_dir):
        ...
        with (
            patch.object(OFOptions, "get_video_reader", return_value=mock_reader),
            patch.object(OFOptions, "get_video_writer", return_value=mock_writer),
            patch(
                "flowreg3d.util.io.factory.get_video_file_writer"
            ) as mock_get_w_writer,
        ):
    ...
    def test_3d_array_output_format(self, fast_3d_of_options):
        ...
        with (
            patch.object(OFOptions, "get_video_reader"),
            patch.object(OFOptions, "get_video_writer", return_value=mock_writer),
            patch(
                "flowreg3d.util.io.factory.get_video_file_writer"
            ) as mock_get_w_writer,
...
class TestErrorHandling3D:
    ...
    def test_3d_executor_instantiation_error(self, fast_3d_of_options):
        ...
        with patch.object(RuntimeContext, "get_parallelization_executor") as mock_get:
            ...
            with patch(
                "flowreg3d.motion_correction.parallelization.sequential_3d.SequentialExecutor3D"
            ):
...
class TestCompensateRecordingIntegration3D:
    ...
    def test_3d_compensate_recording_function(self, fast_3d_of_options):
        ...
        config = RegistrationConfig(
            n_jobs=1, batch_size=5, verbose=True, parallelization="sequential3d"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        options = OFOptions(
            alpha=(1.0, 2.0, 3.0), buffer_size=20, quality_setting="balanced"
        )

        config = RegistrationConfig(
            n_jobs=2, batch_size=8, parallelization="sequential3d"
        )
