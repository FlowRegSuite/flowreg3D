...
from tests.fixtures_3d import create_simple_3d_test_data
...
class TestCompensateArr3DConsistency:
    ...
    def test_3d_consistent_with_file_processing(self, temp_dir):
        ...
        np.mean((video - reference[np.newaxis]) ** 2)
        np.mean((registered_arr - reference[np.newaxis]) ** 2)
    ...
    def test_3d_flow_initialization_chain(self):
        ...
        for t in range(T):
            ...
            for c in range(C):
                ...
                pattern = (
                    np.sin((x - offset_x) * 2 * np.pi / X)
                    * np.sin((y - offset_y) * 2 * np.pi / Y)
                    * np.sin((z - offset_z) * 2 * np.pi / Z)
                ) * 0.5 + 0.5
                video[t, :, :, :, c] = pattern
...
class TestCompensateArr3DEdgeCases:
    ...
    def test_3d_reference_shape_mismatch(self):
        ...
        wrong_reference = np.random.rand(Z + 1, Y, X, C).astype(
            np.float32
        )  # Wrong Z dimension
