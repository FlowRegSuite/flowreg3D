...
class TestAlpha3DValidation:
    ...
    def test_alpha_validation_wrong_length(self):
        ...
        with pytest.raises(
            ValueError, match="Alpha must be scalar, 2-element, or 3-element tuple"
        ):
...
class TestReference3DHandling:
    ...
    def test_reference_frame_indices_3d(self):
        ...
        options = OFOptions(reference_frames=list(range(10)), verbose=False)

        # Mock the 3D compensation function
        with patch(
            "flowreg3d.motion_correction.compensate_arr_3D.compensate_arr_3D"
        ) as mock_compensate:
...
class TestIO3DIntegration:
    ...
    def test_get_video_reader_3d(self):
        ...
        options = OFOptions(input_file=test_volume, buffer_size=15, bin_size=1)
        ...
        with patch("flowreg3d.util.io.factory.get_video_file_reader") as mock_factory:
            ...
            mock_factory.assert_called_once_with(
                test_volume, buffer_size=15, bin_size=1, dim_order="TZYX"
            )
...
class TestSaveLoad3D:
    ...
    def test_save_load_with_3d_reference(self, temp_dir):
        ...
        options = OFOptions(reference_frames=ref_volume, output_path=temp_dir)
...
class TestConvenienceFunction3D:
    ...
    def test_compensate_inplace_3d(self):
        ...
        with (
            patch("flowreg3d.get_displacement") as mock_get_disp,
            patch(
                "flowreg3d.motion_correction.compensate_arr_3D.compensate_arr_3D"
            ) as mock_compensate,
        ):
