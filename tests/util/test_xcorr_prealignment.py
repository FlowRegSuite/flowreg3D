...
def test_sign_convention():
    ...
    print(f"Moved point at:     Z=10, Y=28, X=22")
    print(f"Movement: Y+3, X-3 (point moved down and left)")
    ...
    assert np.abs(aligned_point_pos[0] - expected_pos[0]) <= 1 and \
           np.abs(aligned_point_pos[1] - expected_pos[1]) <= 1 and \
           np.abs(aligned_point_pos[2] - expected_pos[2]) <= 1, \
