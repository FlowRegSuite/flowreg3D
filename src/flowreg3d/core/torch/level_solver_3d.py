...
def _as_W_like(weight_level, like):
    if isinstance(weight_level, (float, int)):
        return torch.as_tensor(weight_level, dtype=like.dtype, device=like.device).view(1, 1, 1, 1)
    if not isinstance(weight_level, torch.Tensor):
        weight_level = torch.as_tensor(weight_level, dtype=like.dtype, device=like.device)
    ...
    if weight_level.dim() == 4 and tuple(weight_level.shape) == tuple(like.shape):
        return weight_level
    raise ValueError("weight_level must be (p,m,n,C), (C,), scalar, or convertible to such")


def level_solver_rbgs3d_torch(J11, J22, J33, J44, J12, J13, J23, J14, J24, J34, weight_level, u, v, w, alpha,
        iterations, update_lag, a_data, a_smooth, hx, hy, hz, omega=1.95, eps=1e-6, ):
    with torch.no_grad():
        P, M, N, C = J11.shape
        ...
        if isinstance(a_data, (float, int)):
            A_vec = torch.full((1, 1, 1, C), float(a_data), dtype=J11.dtype, device=J11.device)
        else:
            A_vec = torch.as_tensor(a_data, dtype=J11.dtype, device=J11.device).view(1, 1, 1, -1)
        ...
        for it in range(iterations):
            ...
            if upd_tick:
                ...
                dv3 = dv.unsqueeze(-1)
                dw3 = dw.unsqueeze(-1)
                E = (J11 * (du3 * du3) + J22 * (dv3 * dv3) + J33 * (dw3 * dw3) + 2 * J12 * (du3 * dv3) + 2 * J13 * (
                            du3 * dw3) + 2 * J23 * (dv3 * dw3) + 2 * J14 * du3 + 2 * J24 * dv3 + 2 * J34 * dw3 + J44)
                E.clamp_min_(0)
                ...
                if a_smooth != 1:
                    ...
                    mag = ux * ux + uy * uy + uz * uz + vx * vx + vy * vy + vz * vz + wx * wx + wy * wy + wz * wz
                    psi_smooth.zero_()
            ...
            num_u[cK, cJ, cI] = (
                    nu_data[cK, cJ, cI] + wXm * (u[cK, cJ, 0:N - 2] + du[cK, cJ, 0:N - 2] - u[cK, cJ, cI]) + wXp * (
                        u[cK, cJ, 2:N] + du[cK, cJ, 2:N] - u[cK, cJ, cI]) + wYm * (
                                u[cK, 0:M - 2, cI] + du[cK, 0:M - 2, cI] - u[cK, cJ, cI]) + wYp * (
                                u[cK, 2:M, cI] + du[cK, 2:M, cI] - u[cK, cJ, cI]) + wZm * (
                                u[0:P - 2, cJ, cI] + du[0:P - 2, cJ, cI] - u[cK, cJ, cI]) + wZp * (
                                u[2:P, cJ, cI] + du[2:P, cJ, cI] - u[cK, cJ, cI]))
            den_u[cK, cJ, cI] = denom_u_data[cK, cJ, cI] + (wXm + wXp + wYm + wYp + wZm + wZp)

            num_v[cK, cJ, cI] = (
                    nv_data[cK, cJ, cI] + wXm * (v[cK, cJ, 0:N - 2] + dv[cK, cJ, 0:N - 2] - v[cK, cJ, cI]) + wXp * (
                        v[cK, cJ, 2:N] + dv[cK, cJ, 2:N] - v[cK, cJ, cI]) + wYm * (
                                v[cK, 0:M - 2, cI] + dv[cK, 0:M - 2, cI] - v[cK, cJ, cI]) + wYp * (
                                v[cK, 2:M, cI] + dv[cK, 2:M, cI] - v[cK, cJ, cI]) + wZm * (
                                v[0:P - 2, cJ, cI] + dv[0:P - 2, cJ, cI] - v[cK, cJ, cI]) + wZp * (
                                v[2:P, cJ, cI] + dv[2:P, cJ, cI] - v[cK, cJ, cI]))
            den_v[cK, cJ, cI] = denom_v_data[cK, cJ, cI] + (wXm + wXp + wYm + wYp + wZm + wZp)

            num_w[cK, cJ, cI] = (
                    nw_data[cK, cJ, cI] + wXm * (w[cK, cJ, 0:N - 2] + dw[cK, cJ, 0:N - 2] - w[cK, cJ, cI]) + wXp * (
                        w[cK, cJ, 2:N] + dw[cK, cJ, 2:N] - w[cK, cJ, cI]) + wYm * (
                                w[cK, 0:M - 2, cI] + dw[cK, 0:M - 2, cI] - w[cK, cJ, cI]) + wYp * (
                                w[cK, 2:M, cI] + dw[cK, 2:M, cI] - w[cK, cJ, cI]) + wZm * (
                                w[0:P - 2, cJ, cI] + dw[0:P - 2, cJ, cI] - w[cK, cJ, cI]) + wZp * (
                                w[2:P, cJ, cI] + dw[2:P, cJ, cI] - w[cK, cJ, cI]))
            den_w[cK, cJ, cI] = denom_w_data[cK, cJ, cI] + (wXm + wXp + wYm + wYp + wZm + wZp)
